...
#define CAN1_TX_EXT_ID    0x10000101U
#define CAN1_TX_PERIOD_MS 500U
#define ADC_HALF_SCALE    2047U
...
int main(void)
{
  ...
  canFilter.FilterIdHigh = 0U;
  canFilter.FilterIdLow = 0U;
  canFilter.FilterMaskIdHigh = 0U;
  canFilter.FilterMaskIdLow = 0U;
  canFilter.FilterFIFOAssignment = CAN_RX_FIFO0;
  canFilter.FilterActivation = ENABLE;
  canFilter.SlaveStartFilterBank = 14U;

  canFilter.FilterBank = 14U;
  ...
  while (1)
  {
    ...
    if (HAL_ADC_Start(&hadc1) == HAL_OK)
    {
      if (HAL_ADC_PollForConversion(&hadc1, 1U) == HAL_OK)
      {
        HAL_GPIO_WritePin(LD1_GPIO_Port, LD1_Pin,
                          (HAL_ADC_GetValue(&hadc1) > ADC_HALF_SCALE) ?
    ...
    {
      txHeader.ExtId = CAN1_TX_EXT_ID;
      (void)HAL_CAN_AddTxMessage(&hcan1, &txHeader, txData, &txMailbox);
      nextTxTick = currentTick + CAN1_TX_PERIOD_MS;
    }
